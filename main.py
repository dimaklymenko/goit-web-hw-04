...
def save_data_from_form(data):
    ...
    try:
        CURRENT_TIME = datetime.now().isoformat(sep=' ', timespec='microseconds')  # Точний формат часу
        # Створення нового запису
        new_entry = {key: value for key, value in [el.split('=') for el in parse_data.split('&')]}
        file_path = 'storage/data.json'
        # Читання існуючого словника з файлу
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                data_dict = json.load(file)  # Завантажити існуючий словник
        except (FileNotFoundError, json.JSONDecodeError):
            data_dict = {}  # Якщо файл не існує або некоректний, створити новий словник
        # Додавання нового запису
        data_dict[CURRENT_TIME] = new_entry
        ...
        #     parse_dict = {CURRENT_TIME.strftime('%Y-%m-%d %H:%M:%S') : {key: value for key, value in [el.split('=') for el in parse_data.split('&')]}}
